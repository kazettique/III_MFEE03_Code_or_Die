...
<script>
    ...
    let data = [];
    const getLocalData = () => {
    fetch("test_list_api.php").then(res => res.json()).then(json => {
      renderDOM(json);
    })
    ...
    const renderHtml = (data) => {
      let html = ""
      data.map(obj => {
        return html += `<tr><td><img src="./upload/${obj["imgname"]}"></td><td><a href="editimg.php?sid=${obj["sid"]}">edit</a></td><td>${obj["title"]}</td><td>${obj["type"]}</td><td><a href="editor2.php?sid=${obj["sid"]}">edit</a></td><td><a href="__delete.php?sid=${obj["sid"]}" class="delete">delete</a></td></tr>` ;
      })
      info.innerHTML = html;
    }
    ...
    window.addEventListener("hashchange", () => {fetch("__list1.php").then(res => res.json()).then(json => {
      renderDOM(json);})});
